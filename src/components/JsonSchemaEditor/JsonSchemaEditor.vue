		...
		<aRow class="row" :gutter="10">
			<aCol :span="8" class="ant-col-name">
				<div :style="{ marginLeft: `${20 * deep}px` }" class="ant-col-name-c">
					<aButton
						...
						@click="hidden = !hidden">
						<template #icon>
							<xIcon icon="CaretRightOutlined" v-if="hidden" />
							<xIcon icon="CaretDownOutlined" v-else />
						</template>
					</aButton>
			...
			<aCol :span="4">
				<aSelect
					...
					">
					<aSelectOption :key="t" v-for="t in TYPE_NAME">
						{{ t }}
					</aSelectOption>
				</aSelect>
			</aCol>
			...
			<aCol :span="6" class="ant-col-setting">
				<aTooltip>
					...
					<aButton type="link" class="setting-icon" @click="onSetting">
						<template #icon><xIcon icon="SettingOutlined" /></template>
					</aButton>
				</aTooltip>
				<aTooltip v-if="isObject">
					...
					<aButton type="link" class="plus-icon" @click="addChild">
						<template #icon><xIcon icon="PlusOutlined" /></template>
					</aButton>
				</aTooltip>
					...
					<aCol :span="8" v-for="(item, key) in advancedValue" :key="key">
						<aFormItem>
							...
							<aSelect
								...
								<aSelectOption :key="t" v-for="t in advancedAttr[key].enums">
									{{ t }}
								</aSelectOption>
							</aSelect>
							<aInput
					...
					<aCol :span="8" v-for="item in customProps" :key="item.key">
						<aFormItem :label="item.key">
							...
							<aButton
								...
								style="width: 30px">
								<template #icon><xIcon icon="CloseOutlined" /></template>
							</aButton>
						</aFormItem>
					</aCol>
					...
					<aCol :span="8">
						<aFormItem>
							<aButton
								...
								v-if="customing">
								<template #icon><xIcon icon="CheckOutlined" /></template>
							</aButton>
