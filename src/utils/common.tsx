...
export const randomValueAndProp = (obj: any) => {
    if (vUtils.isArray(obj) && obj.length > 0) {
        const start = 0;
        const end = obj.length;
        const key = Math.floor(Math.random() * end + start);
        return [obj[key], key];
    } else if (vUtils.isPlainObject(obj)) {
...
export const arrayChangeIndex = (arr, dragId, dropId) => {
    ...
    const dragItem = vUtils.find(arr, findBy);
    const dragIndex = vUtils.findIndex(arr, findBy);
    const dropIndex = vUtils.findIndex(arr, {_id: dropId});
